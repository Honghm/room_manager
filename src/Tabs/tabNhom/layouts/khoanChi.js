  ...
  getIcon(value){
   ...
   getData() {
    ...
    this.itemRef.ref('Users').on('child_added', (dataSnapshot)=>{
        var dataUserInGroup = [];
       if(dataSnapshot.child('idNhom').val()== 1 ){
        dataUserInGroup.push({
            label: dataSnapshot.child('ten').val(),
            value: dataSnapshot.key,
        })
        this.setState(
            this.state.dataNguoiThamGia  = this.state.dataNguoiThamGia.concat(dataUserInGroup),
        );
       }
    })
   ...
   themKhoanChi = () => {
    ...
    render(){
        ...
        return(
            ...
            <SlidingUpPanel
                       ref={c => this._panel = c}
                       draggableRange = {{top: height - 170, bottom: 50 }}
                       ...
                       >
                           ...
                           <View style = {{height: height - 230, backgroundColor: 'white', borderRadius: 24, justifyContent: 'center', alignItems: 'center', flexDirection:'column-reverse'}}>
                             
                              {/* Buttom điều hướng */}
                              <View style = {{width: width - 60, padding: 10, flexDirection: 'row', justifyContent: 'space-between'}}>
                                   <TouchableOpacity
                               ...
                               {/* Ghi chú */}
                               <View style = {{width: width - 60, paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between'}}>
                                 ...
                                 <View style = {{width: width - 60, paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between',}}>
                                   ...
                                   <DropDownPicker
                                    items={this.state.dataNguoiThamGia}
                                    multiple
                                    defaultValue={0}
                                    ...
                                    onChangeItem={item => this.setState({
                                        nguoiThamGia: this.state.nguoiThamGia = this.state.nguoiThamGia.concat(item)
                                    })}
